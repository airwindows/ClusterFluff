...
func _physics_process(delta):
	...
	if (control_key == $"..".controlled && shiftitude == $"..".shiftituded && remainingTimingBalls > 0):
		...
		if (Input.is_action_pressed("ui_right")):
			...
			self.angular_velocity *= (1.0-delta)
			beingcontrolled = true
		if (Input.is_action_pressed("ui_accept") && $"../../Globals".kabonus > 1.0):
			$"../../Globals".kabonus -= 1.0
	...
	if (self.position.y < -535 || self.position.y > 560 || self.position.x < -980 || self.position.x > 980):
		...
		$"../../Globals".kabonus += remainingTimingBalls * 0.005
		$"../../Globals".factor = 300 + (sqrt(floor($"../../Globals".kabonus)*0.1)*300)
		self.queue_free()
